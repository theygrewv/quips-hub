...
export default function App() {
  ...
  const login = async () => {
    ...
    try { await client?.signIn(h); } catch (e) { alert("AUTH_ERR"); }
  };

  if (loading) return <div style={fs}>[ BOOTING_QUIPS_OS... ]</div>;

  if (view === 'bats') return (
    <div style={fs}>
      <h1 style={{color:'#0f0'}}>[ BATS ]</h1>
      <p>Bilateral Analytics & Tracking System: ONLINE</p>
      <button onClick={() => setView('hub')} style={sBtn}>RETURN</button>
    </div>
  );
  ...
  if (view === 'germ') return (
    <div style={{...fs, color:'#f0f'}}>
      <h1>[ GERM_NETWORK ]</h1>
      <div style={{border:'1px solid #f0f', padding:'20px', textAlign:'left', width:'80%'}}>
        <p>> P2P_PROTOCOL: ARMORED_STREAM</p>
        <p>> PEER_DISCOVERY: ACTIVE</p>
        <p>> MSG_ENCRYPTION: AES-256-GCM</p>
        <hr style={{borderColor:'#f0f'}} />
        <p style={{fontSize:'0.8rem'}}>Waiting for encrypted p2p handshake...</p>
      </div>
      <button onClick={() => setView('hub')} style={{...sBtn, backgroundColor:'#f0f'}}>RETURN</button>
    </div>
  );
...
const sBtn = { padding: '10px 20px', background: '#0f0', color: '#000', border: 'none', cursor: 'pointer', marginTop: '20px' };
